...
- SAE/transcoder work remains gated: sparse features should wait until module
  ...
  baseline is now `pca_rank64`, with the mean-delta direction as the minimal
  rank-1 partial repair.
...
PCA rank 64 the current cleanest compressed repair.
...
- Replace or augment the generation scorer with contradiction and
  unsafe-continuation detection before larger sweeps.
- Add held-out harmful prompts and more adversarial benign prompts before
  declaring the PCA repair robust.
- Decide whether SAE/transcoder probes should target raw MLP outputs, MLP
  deltas, or donor-recipient paired activations.
- Use PCA rank 64 and mean-delta patching as RQ0 comparators for any sparse
  feature patching result.
