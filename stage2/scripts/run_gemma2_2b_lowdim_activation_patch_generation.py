#!/usr/bin/env python3
"""Low-dimensional generation checks for the Gemma-2-2B activation repair."""
...
from run_gemma2_2b_activation_patch_target_loss import MODEL_IDS, make_batches  # noqa: E402
...
RESULT_DIR = ROOT / "stage2" / "results" / "gemma2_2b_lowdim_activation_patch_generation"
...
def parse_variant(raw: str) -> tuple[str, object, str, int | str]:
    if raw == "full":
        return "full", None, "full_mlp", "full"
    if raw == "mean_delta":
...
def write_summary(path: Path, rows: list[dict[str, object]], layers: tuple[int, ...]) -> None:
    lines = [
        "# Gemma-2-2B Low-Dimensional Activation Patch Generation",
        "",
        f"Greedy generation check for compressed approximations to the successful MLP activation patch over layers `{layers[0]}-{layers[-1]}`.",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--variants", default="full,mean_delta,pca_rank16,pca_rank64,top_neuron_k256,top_neuron_k512,random_rank64")
...
def main() -> int:
    ...
    if not args.skip_baselines:
        eval_variants.extend([("base", None, None, "base"), ("abliterated", None, None, "abliterated")])
    eval_variants.extend((kind, param, label, label) for kind, param, label, _dim in variants)

    for kind, param, label, model_name in eval_variants:
    ...
    metrics_path = args.result_dir / "gemma2_2b_lowdim_activation_patch_generation_metrics.csv"
    records_path = args.result_dir / "gemma2_2b_lowdim_activation_patch_generation_records.jsonl"
    summary_path = args.result_dir / "GEMMA2_2B_LOWDIM_ACTIVATION_PATCH_GENERATION_SUMMARY.md"
    manifest_path = args.result_dir / "manifest.json"
    write_csv(metrics_path, summary_rows)
    write_jsonl(records_path, all_records)
    write_summary(summary_path, summary_rows, layers)
