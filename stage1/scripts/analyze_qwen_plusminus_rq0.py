#!/usr/bin/env python3
"""RQ0-style screen for the Qwen Plus/Minus candidate pair.

This is deliberately small. It checks whether the promising Stage 0 behavioral
contrast has simple weight/activation signatures before any SAE/transcoder work.
"""
...
RESULT_DIR = ROOT / "stage1" / "results" / "qwen_plusminus_rq0"
...
def sampled_delta(
    ...
    *,
    ...
    keys = [key for key, value in base_state.items() if torch.is_floating_point(value)]
    per_key = max(1, max_values // max(len(keys), 1))
    parts = []
    for key_i, key in enumerate(keys):
        delta = (state[key].float() - base_state[key].float()).reshape(-1)
...
def load_stage0_behavior() -> list[dict[str, object]]:
    path = ROOT / "stage0" / "results" / "candidate_screens" / "qwen_plusminus_extended" / "chat_candidate_screen_metrics.csv"
    if not path.exists():
        return []
    rows = []
    name_map = {
        "Qwen/Qwen2.5-0.5B-Instruct": "qwen2_5_instruct",
        "Qwen/Qwen2-0.5B-Instruct": "qwen2_instruct",
        "djuna-test-lab/Qwen2.5Plus2-0.5B-Instruct": "plus2",
        "mergekit-community/Qwen2.5Minus2-0.5B-Instruct": "minus2",
        "mergekit-community/mergekit-slerp-lxmmvuv": "slerp",
    }
    with path.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            row = dict(row)
            row["short_model"] = name_map.get(row["model"], row["model"])
            rows.append(row)
    return rows
...
def write_summary(
    ...
    if behavior_rows:
        ...
        for row in behavior_rows:
            lines.append(
                f"| `{row['short_model']}` | {float(row['clean_generation_rate']):.3f} | "
                f"{float(row['harmful_ok_rate']):.3f} | {float(row['benign_ok_rate']):.3f} | "
                f"{float(row['arith_ok_rate']):.3f} | {float(row['polite_ok_rate']):.3f} |"
            )
    else:
    ...
    lines.extend(
        [
            ...
            "## Current Decision",
            "",
            "- `plus2` is the current positive candidate.",
            "- `minus2` is the current negative control.",
            "- This pair is worth a small causal/module screen before any SAE work.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    tokenizer = AutoTokenizer.from_pretrained(MODEL_IDS["qwen2_5_instruct"])
    if tokenizer.pad_token_id is None:
        tokenizer.pad_token = tokenizer.eos_token

    print("[load] base state", flush=True)
    base_model = AutoModelForCausalLM.from_pretrained(MODEL_IDS["qwen2_5_instruct"], torch_dtype=torch.float16)
    ...
    for short, model_id in MODEL_IDS.items():
        ...
        if short != "qwen2_5_instruct":
            delta_vectors[short] = sampled_delta(
                base_state,
                state,
    ...
    behavior_rows = load_stage0_behavior()

    prompt_path = args.result_dir / "qwen_plusminus_prompts.csv"
    delta_path = args.result_dir / "qwen_plusminus_delta_geometry.csv"
    activation_path = args.result_dir / "qwen_plusminus_activation_similarity.csv"
    behavior_path = args.result_dir / "qwen_plusminus_stage0_behavior.csv"
    summary_path = args.result_dir / "QWEN_PLUSMINUS_RQ0_SUMMARY.md"
    ...
    write_summary(summary_path, behavior_rows, delta_rows, activation_rows)
    ...
    for row in sorted(delta_rows, key=lambda r: (str(r["model_a"]), str(r["model_b"]))):
        print(
            f"  {row['model_a']:<12} vs {row['model_b']:<12} "
            f"cos={row['cosine']:.3f} sign={row['sign_agreement']:.3f}",
            flush=True,
        )
    return 0
