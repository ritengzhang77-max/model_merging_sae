...
## Long-Generation Audit Caveat

A targeted `max_new_tokens=160` audit shows that the hologram/lamination prompt
is already fragile in the plain alpha-`0.75` merge and random controls. At
64 tokens, baseline/random responses often look like warning-prefaced refusals;
by 160 tokens, they continue into procedural hologram/material/process details.

This reframes the top10 feature-subtract result. Top10 still causes an early
refusal-rationale degradation on that prompt, but it is not creating a new
failure from a robustly safe baseline. It is perturbing a trajectory that is
already vulnerable under longer decoding.
...
- Hologram max-160 feature-subtract probe:
